...
export class SendMoneyPage {
  ...
  private readonly msisdnInput = this.page.getByRole('textbox', { name: 'Enter target MSISDN' });
  ...
  async goto() {
    await this.page.goto('/outgoing/send-money');
    await this.page.waitForLoadState('networkidle');
    await this.page.getByText('Account BalanceACTIVE').click();
    await this.page.getByRole('button', { name: 'Outgoing' }).click();
    await this.page.getByRole('link', { name: 'Send Money' }).click();
    await this.page.waitForLoadState('networkidle');
  }

  async fillTransferDetails(msisdn: string, amount: string) {
    await this.msisdnInput.click();
  ...
  async sendMoney(msisdn: string, amount: string, mpin: string) {
    await this.fillTransferDetails(msisdn, amount);
