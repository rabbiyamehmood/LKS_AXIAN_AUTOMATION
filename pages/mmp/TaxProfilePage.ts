...
export class TaxProfilePage {
  constructor(readonly page: Page) {}

  // ── Navigation ────────────────────────────────────────────────────────────

  async navigateToTaxProfileList() {
    await this.page.getByRole('button', { name: 'Merchant Onboarding' }).click();
    await this.page.getByRole('link', { name: 'Tax Profile List' }).click();
    await expect(this.page.getByRole('button', { name: 'Tax Profile Add' })).toBeVisible({ timeout: 15_000 });
  }
  ...
  async createProfile(data: {
    ...
    await this.page.getByRole('button', { name: 'Tax Profile Add' }).click();
    await expect(this.page.getByRole('heading', { name: 'Add Tax Profile' })).toBeVisible({ timeout: 10_000 });
    ...
    await this.page.getByText(data.region, { exact: true }).click();

    await this.page.getByRole('button', { name: 'Add Tax Rule' }).click();
    await this.page.getByRole('textbox', { name: 'Enter tax value' }).fill(String(data.taxValue));
  ...
  async checkerReview(
    loginPage: LoginPage,
    requestType: 'TAX PROFILE CREATION' | 'TAX PROFILE UPDATE',
  ...
  async openFilterPanel() {
    ...
    await expect(
      this.page.getByPlaceholder('Search by profile name')
    ).toBeVisible({ timeout: 10_000 });
  }

  async filterByProfileName(name: string) {
    await this.page.getByPlaceholder('Search by profile name').fill(name);
    await this.page.getByRole('button', { name: 'Apply Filters' }).click();
    await this.page.waitForTimeout(800);
  }

  async filterByRegion(region: string) {
    await this.page.getByPlaceholder('Search by region').fill(region);
    await this.page.getByRole('button', { name: 'Apply Filters' }).click();
    await this.page.waitForTimeout(800);
  }
  ...
  async assertRegionInputEmpty() {
    await expect(
      this.page.getByPlaceholder('Search by region')
    ).toHaveValue('');
  }
}
