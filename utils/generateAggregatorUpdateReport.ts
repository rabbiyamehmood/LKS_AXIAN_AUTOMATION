...
interface AggregatorUpdateTestCase {
  tcId: string;
  module: string;
  role: string;
...
const testCases: AggregatorUpdateTestCase[] = [

  // ── POSITIVE ──────────────────────────────────────────────────────────────────

  {
    tcId: 'TC_AGG_UPD_E2E_001',
    module: 'Aggregator Update & Approval',
    role: 'AdminMaker → AdminChecker',
    testType: 'Positive',
    description: 'Full end-to-end aggregator update and approval flow',
    preconditions: 'At least one aggregator exists in Aggregator List. AdminMaker & AdminChecker accounts are active.',
    testData:
      'Updated Name: Auto_Aggregator_Update_<timestamp>\nUpdated Email: aggregatorupdate<timestamp>@yopmail.com\nApproval Comment: Approved by AdminChecker - Automation Test',
    testSteps:
      '1. Navigate to /login\n2. Login as AdminMaker\n3. Go to Aggregator Management → Aggregator List\n4. Click Edit on the first aggregator\n5. Clear Aggregator Name and fill with unique updated name\n6. Clear Email and fill with unique updated email\n7. Click Save\n8. Verify "Processed OK" toast appears\n9. Close toast and Logout\n10. Login as AdminChecker\n11. Go to Inbox → Pending Processes\n12. Click AGGREGATOR UPDATE row\n13. Click Review\n14. Verify "Review Aggregator" heading is visible\n15. Click Approve\n16. Enter approval comment\n17. Click Confirm\n18. Verify "Process approved successfully" toast\n19. Close toast and Logout',
    expectedResult:
      'Aggregator updated with "Processed OK" toast.\nAdminChecker sees it in Pending Processes as AGGREGATOR UPDATE.\nApproval succeeds with "Process approved successfully" toast.\nBoth users logged out.',
    status: 'N/A',
  },

  // ── NEGATIVE — MAKER EDIT FORM VALIDATION ─────────────────────────────────────

  {
    tcId: 'TC_AGG_UPD_NEG_001',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
    description: 'Clear Aggregator Name — shows "Aggregator name is required"',
    preconditions: 'AdminMaker logged in. Edit Aggregator form is open.',
    testData: 'Aggregator Name: (cleared)',
    testSteps:
      '1. Login as AdminMaker\n2. Go to Aggregator Management → Aggregator List\n3. Click Edit on first aggregator\n4. Clear the Aggregator Name field\n5. Click Save\n6. Assert "Aggregator name is required" is visible\n7. Assert "Processed OK" is NOT visible',
    expectedResult: '"Aggregator name is required" error shown. No update submitted.',
    status: 'N/A',
  },
  {
    tcId: 'TC_AGG_UPD_NEG_002',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
    description: 'Clear Contact Person — shows "Contact person name is required"',
    preconditions: 'AdminMaker logged in. Edit Aggregator form is open.',
    testData: 'Contact Person: (cleared)',
    testSteps:
      '1. Login as AdminMaker\n2. Open Edit form for first aggregator\n3. Clear the Contact Person field\n4. Click Save\n5. Assert "Contact person name is required" is visible\n6. Assert "Processed OK" is NOT visible',
    expectedResult: '"Contact person name is required" error shown. No update submitted.',
    status: 'N/A',
  },
  {
    tcId: 'TC_AGG_UPD_NEG_003',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
  ...
  },
  {
    ...
    testSteps:
      '1. Login as AdminMaker\n2. Open Edit form for first aggregator\n3. Clear Email and enter "invalidemail.com"\n4. Click Save\n5. Assert "Invalid email format" is visible\n6. Assert "Processed OK" is NOT visible',
    expectedResult: '"Invalid email format" error shown. No update submitted.',
    status: 'N/A',
  },
  {
    tcId: 'TC_AGG_UPD_NEG_005',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
  ...
  {
    tcId: 'TC_AGG_UPD_NEG_006',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
    description: 'Non-numeric phone — field rejects alphabets, stays empty, shows "Phone number is required"',
    preconditions: 'AdminMaker logged in. Edit Aggregator form is open.',
    testData: 'Phone: ABCDEFGH (non-numeric)',
    testSteps:
      '1. Login as AdminMaker\n2. Open Edit form for first aggregator\n3. Clear Phone and type "ABCDEFGH"\n4. Assert phone field value is empty (field rejects alphabets)\n5. Click Save\n6. Assert "Phone number is required" is visible\n7. Assert "Processed OK" is NOT visible',
    expectedResult: 'Phone field rejects alphabets silently → field stays empty → "Phone number is required" shown.',
    status: 'N/A',
  },
  {
    tcId: 'TC_AGG_UPD_NEG_007',
    module: 'Aggregator Update',
    role: 'AdminMaker',
    testType: 'Negative',
    description: 'Uncheck QR (no payment method) — shows "At least one payment method must be selected"',
    preconditions: 'AdminMaker logged in. Edit Aggregator form is open. QR is checked.',
    testData: 'QR: unchecked',
    testSteps:
      '1. Login as AdminMaker\n2. Open Edit form for first aggregator\n3. Uncheck QR payment method\n4. Click Save\n5. Assert "At least one payment method must be selected" is visible\n6. Assert "Processed OK" is NOT visible',
    expectedResult: '"At least one payment method must be selected" error shown. No update submitted.',
    status: 'N/A',
  },
  ...
  {
    tcId: 'TC_AGG_UPD_NEG_008',
    module: 'Aggregator Update Approval',
    role: 'AdminChecker',
    testType: 'Negative',
    description: 'Checker cannot Approve update without a comment — Confirm is disabled',
    preconditions: 'A pending AGGREGATOR UPDATE exists in Pending Processes.',
    testData: 'Comment: (empty)',
    testSteps:
      '1. AdminMaker submits an aggregator update\n2. Login as AdminChecker\n3. Go to Inbox → Pending Processes\n4. Click AGGREGATOR UPDATE row\n5. Click Review\n6. Click Approve\n7. Leave Comments field empty\n8. Assert Confirm button is disabled\n9. Assert "Process approved successfully" is NOT visible',
    expectedResult: 'Confirm button is disabled when comment is empty. Approval cannot proceed.',
    status: 'N/A',
  },
  {
    tcId: 'TC_AGG_UPD_NEG_009',
    module: 'Aggregator Update Approval',
    role: 'AdminChecker',
    testType: 'Negative',
    description: 'Checker cannot Reject update without a comment — Confirm is disabled',
    preconditions: 'A pending AGGREGATOR UPDATE exists in Pending Processes.',
    testData: 'Comment: (empty)',
    testSteps:
  ...
  {
    tcId: 'TC_AGG_UPD_NEG_010',
    module: 'Aggregator Update Approval',
    role: 'AdminChecker',
    testType: 'Negative',
    description: 'Checker rejects aggregator update with a valid reason',
    preconditions: 'A pending AGGREGATOR UPDATE exists in Pending Processes.',
    ...
    expectedResult: 'Update is rejected. "Process rejected" toast appears. Request removed from Pending Processes.',
    status: 'N/A',
  },
];
...
async function generateAggregatorUpdateReport() {
  ...
  const sheet = workbook.addWorksheet('Aggregator Update Tests');
  ...
  [
    { metric: 'Module',            value: 'Aggregator Update' },
  ...
  const filePath  = path.join(reportsDir, `Aggregator_Update_Report_${timestamp}.xlsx`);
  await workbook.xlsx.writeFile(filePath);
  console.log(`\n✅ Excel report saved: ${filePath}`);
}

generateAggregatorUpdateReport().catch(err => {
